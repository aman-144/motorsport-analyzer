...
def get_average_time(data):
    ...
    std_Laptime = data['LapTime'].std()
    avg_Sector1Time = data['Sector1Time'].mean()
    avg_Sector2Time = data['Sector2Time'].mean()
    avg_Sector3Time = data['Sector3Time'].mean()
    return avg_Laptime, std_Laptime, avg_Sector1Time, avg_Sector2Time, avg_Sector3Time

ham_avg_Laptime, ham_std_Laptime, ham_avg_S1time, ham_avg_S2time, ham_avg_S3time = get_average_time(ham_data)
ver_avg_Laptime, ver_std_Laptime, ver_avg_S1time, ver_avg_S2time, ver_avg_S3time = get_average_time(ver_data)


print("=== FINGERPRINT COMPARISON ===")
print(f"Avg Lap   — Hamilton: {ham_avg_Laptime} | Verstappen: {ver_avg_Laptime}")
print(f"Std Lap   — Hamilton: {ham_std_Laptime} | Verstappen: {ver_std_Laptime}")
